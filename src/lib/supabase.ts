...
const isMissingScheduledColumnError = (error: any) => {
  const message = `${error?.message || ''} ${error?.details || ''}`.toLowerCase();
  return message.includes('scheduled_at') && (message.includes('does not exist') || message.includes('column') || message.includes('not exist'));
};
...
export const createPost = async (post: any) => {
  ...
  if (!categoryId) {
    const categorySlug = post.category_slug || post.category || post.categoryName || null;
    if (categorySlug && typeof categorySlug === 'string') {
      try {
        ...
        console.warn('Could not resolve category slug for post:', categorySlug, err);
      }
    }
  }
  ...
  const normalizeScheduleTime = (value: any): string | null => {
  ...
  const scheduledAtValue = normalizeScheduleTime(post.scheduled_at || post.scheduledAt || null);
  ...
  const insertData: any = {
    ...
    published_at: status === 'published'
      ...
      : status === 'scheduled' && scheduledAtValue
        ? scheduledAtValue
        : null,
  ...
  if (error && isMissingScheduledColumnError(error) && insertPayload.scheduled_at !== undefined) {
    delete insertPayload.scheduled_at;
...
export const updatePost = async (id: string, post: any) => {
  ...
  if (post.scheduled_at !== undefined) updateData.scheduled_at = post.scheduled_at || null;
  if (post.status !== undefined) {
    const normalizedStatus = post.status;
    updateData.status = normalizedStatus;
    if (normalizedStatus === 'published') {
      updateData.published_at = new Date().toISOString();
    } else if (normalizedStatus === 'scheduled') {
      const scheduledAtValue = post.scheduled_at || post.scheduledAt || null;
      updateData.published_at = scheduledAtValue ? new Date(scheduledAtValue).toISOString() : null;
    } else {
  ...
  if (error && isMissingScheduledColumnError(error) && updateData.scheduled_at !== undefined) {
...
export const getPosts = async (options?: any): Promise<Post[]> => {
  ...
  const categoryFilter = options?.category || options?.category_slug || options?.categorySlug || null;
