...
export default function BulkQueue() {
  ...
  const [localTasks, setLocalTasks] = useState<QueueTask[]>([]);
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (data.tasks) {
        setLocalTasks(prev => {
          const merged = data.tasks.map((t: QueueTask) => {
            const existing = prev.find(p => p.id === t.id);
            return { ...t, showFullText: existing?.showFullText || false };
          });
          return merged;
        });
      }
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm(`Delete ${selectedTaskIds.length} selected task(s) from queue? This cannot be undone.`)) {
      return;
    }
    try {
      ...
      const data = await res.json();
      setMessage(`✅ Deleted ${data.deleted || selectedTaskIds.length} task(s).`);
      setSelectedTaskIds([]);
      fetchStatus();
    } catch (err: any) {
      ...
      <div className="card p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selectedTaskIds.length > 0 && (
            <button onClick={handleDeleteSelected} className="btn btn-error text-xs flex items-center gap-1.5 ml-auto">
              <Trash2 className="w-3.5 h-3.5" />
            ...
            <div key={task.id} className="px-4 py-3 space-y-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 border-b last:border-b-0">
              <div className="flex items-start gap-3">
                <input
                  ...
                  className="mt-1 w-3.5 h-3.5 rounded border-slate-300 text-primary focus:ring-primary"
                />
              ...
              {task.scrapedTextLength && (
                <div className="text-[10px] text-slate-500 bg-slate-50 dark:bg-slate-800 rounded p-2 font-mono">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-slate-400 font-semibold">📄 Scraped: {task.scrapedTextLength.toLocaleString()} chars</span>
                    <span className="text-[9px] text-slate-500">{task.showFullText ? 'Click to collapse' : 'Click to expand full text'}</span>
                  </div>
                  {task.scrapedTextPreview && (
                    <div 
                      className="text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50 rounded p-1 transition-colors"
                      onClick={() => {
                        const newState = localTasks.map(t => 
                          t.id === task.id ? { ...t, showFullText: !t.showFullText } : t
                        );
                        setLocalTasks(newState);
                      }}
                    >
                      {task.showFullText ? (
                        <pre className="whitespace-pre-wrap break-all max-h-[300px] overflow-y-auto text-[9px] leading-relaxed">
                          {task.scrapedTextPreview}{task.scrapedTextPreview.length < (task.scrapedTextLength || 0) ? '\n\n... (truncated)' : ''}
                        </pre>
                      ) : (
                        <div className="line-clamp-3 overflow-hidden">
                          {task.scrapedTextPreview.slice(0, 500)}{task.scrapedTextPreview.length > 500 ? '...' : ''}
                        </div>
                      )}
                    </div>
                  )}
