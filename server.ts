...
function findAdmitCardLink(urls: string[]): string | null {
  // Priority 1: Explicit admit card / hall ticket URLs
  const admitPattern = /admit|hall.?ticket|call.?letter|e.?admit/i;
  const admitCandidates = urls.filter(url => admitPattern.test(url));
  
  // Priority 2: Links with keywords like "admit", "download", "card"
  if (admitCandidates.length > 0) {
    // Prefer official domain
    const officialAdmit = admitCandidates.find(url => isOfficialDomain(url));
    return officialAdmit || admitCandidates[0];
  }
  
  // Priority 3: Any .gov.in / .nic.in link if it contains these words
  const govAdmit = urls.find(url => isOfficialDomain(url) && admitPattern.test(url));
  return govAdmit || null;
}

function findResultLink(urls: string[]): string | null {
  // Priority 1: Explicit result URLs
  const resultPattern = /result|score|marks|merit|outcome/i;
  const resultCandidates = urls.filter(url => resultPattern.test(url));
  
  if (resultCandidates.length > 0) {
    // Prefer official domain
    const officialResult = resultCandidates.find(url => isOfficialDomain(url));
    return officialResult || resultCandidates[0];
  }
  
  // Priority 2: Any .gov.in / .nic.in link if it contains result keywords
  const govResult = urls.find(url => isOfficialDomain(url) && resultPattern.test(url));
  return govResult || null;
}

function getPythonExecutablePath(): string | null {
  const candidates: string[] = [];
  const venvPython = path.join(path.resolve(process.cwd(), '..'), '.venv', process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python');
  ...
  return candidates.find(candidate => candidate && candidate.trim().length > 0) || null;
}

async function runPythonScraper(url: string) {
  const scriptPath = path.join(process.cwd(), 'scrape_url.py');
  ...
  const outputDir = path.join(process.cwd(), 'python_output');

  try {
    const { stdout } = await execFileAsync(pythonExecutable, [scriptPath, url, '--output-dir', outputDir], {
      cwd: process.cwd(),
      timeout: 90000,
    ...
    return {
      ...parsed,
      method: 'python-scraper' as const,
    };
  } catch (err: any) {
    console.log(`[Scraper] Python fallback failed for ${url}: ${err.message}`);
    return null;
  }
}
...
function fallbackBulletGenerator(documentText: string, userInstructions?: string, sourceUrl?: string) {
  ...
  const admitCardLink = findAdmitCardLink(scrapedUrls) || '';
  const resultLink = findResultLink(scrapedUrls) || '';
...
app.post('/api/scrape-website', async (req, res) => {
  try {
    ...
    const pythonScrapeResult = await runPythonScraper(targetUrl);
    if (pythonScrapeResult?.text && typeof pythonScrapeResult.text === 'string' && pythonScrapeResult.text.trim().length > 200) {
      console.log(`[Scraper] Python scraper succeeded for ${targetUrl}`);
      return res.json({
        text: pythonScrapeResult.text,
        method: 'python-scraper',
        isFallback: false,
        title: pythonScrapeResult.title,
        savedFiles: pythonScrapeResult.saved_files,
      });
    }
...
app.post('/api/generate-bulletin', async (req, res) => {
  ...
  try {
    ...
    // Extract useful links from scraped text to pass to AI
    const scrapedUrls = extractUrlsFromText(documentText);
    const sourceApplyLink = findApplyLink(scrapedUrls, sourceUrl) || '';
    const sourceNotificationLink = findNotificationLink(scrapedUrls) || '';
    const sourceOfficialWebsite = findOfficialWebsite(scrapedUrls, sourceUrl) || '';
    const sourceAdmitCardLink = findAdmitCardLink(scrapedUrls) || '';
    const sourceResultLink = findResultLink(scrapedUrls) || '';

    // Build a links summary to feed into the AI prompt
    let linksContext = '';
    if (scrapedUrls.length > 0) {
      linksContext = `\n\n=== EXTRACTED LINKS FROM THE SCRAPED PAGE (USE THESE FOR LINK FIELDS) ===\n`;
      linksContext += `All links found on page: ${scrapedUrls.join(', ')}\n`;
      if (sourceApplyLink) linksContext += `Best apply link candidate: ${sourceApplyLink}\n`;
      if (sourceNotificationLink) linksContext += `Best notification PDF link candidate: ${sourceNotificationLink}\n`;
      if (sourceOfficialWebsite) linksContext += `Best official website candidate: ${sourceOfficialWebsite}\n`;
      if (sourceAdmitCardLink) linksContext += `Best admit card link candidate: ${sourceAdmitCardLink}\n`;
      if (sourceResultLink) linksContext += `Best result link candidate: ${sourceResultLink}\n`;
      linksContext += `Source URL: ${sourceUrl || 'N/A'}\n`;
      linksContext += `\nIMPORTANT: Use these extracted links to fill ALL 6 link fields: apply_link, notification_link, official_website, admit_card_link, result_link. Prefer .gov.in/.nic.in links. If a specific link type is not found, use empty string "".`;
    }
...
- admit_card_link: Admit card / hall ticket download link (agar hai to .gov.in ya empty "")
- result_link: Result download link (agar hai to .gov.in ya empty "")
...
let autoSchedulerIntervalMs = 60 * 60 * 1000;
let scheduledPublisherTimer: NodeJS.Timeout | null = null;
...
async function publishScheduledPosts() {
  ...
  try {
    ...
    const duePosts = (data || []).filter(post => {
      const target = post?.published_at;
    ...
    console.log(`[Scheduler] Published ${ids.length} scheduled posts.`);
  } catch (err: any) {
    console.error('[Scheduler] Failed to publish scheduled posts:', err?.message || err);
  }
}

function startScheduledPostPublisher(intervalMs = 60 * 1000) {
  if (scheduledPublisherTimer) {
    clearInterval(scheduledPublisherTimer);
  }
  scheduledPublisherTimer = setInterval(() => {
...
function startAutoScheduler(intervalMs = 60 * 60 * 1000) {
  stopAutoScheduler();
  autoSchedulerEnabled = true;
  autoSchedulerIntervalMs = intervalMs;
  schedulerTimer = setInterval(() => {
    const state = loadQueue();
    const hasPending = state.tasks.some(task => task.status === 'pending' || task.status === 'failed');
    if (!hasPending || state.isPaused || state.cancelRequested) return;
    console.log(`[Scheduler] Auto-run triggered for pending queue tasks.`);
    void processQueue();
  }, intervalMs);
}

function applyAutoSchedulerState() {
  if (autoSchedulerEnabled) {
    startAutoScheduler(autoSchedulerIntervalMs);
  } else {
    stopAutoScheduler();
  }
}

function canRetryTask(task: QueueTask): boolean {
...
async function processQueueTask(task: QueueTask, instructions: string = '') {
  ...
  try {
    ...
    const desiredStatus = task.postStatus === 'published' ? 'published' : task.postStatus === 'scheduled' ? 'scheduled' : 'draft';
    const scheduledAtRaw = desiredStatus === 'scheduled' ? (task.scheduledAt || new Date().toISOString()) : null;
    const scheduledAt = scheduledAtRaw ? new Date(scheduledAtRaw).toISOString() : null;
    const publishAt = desiredStatus === 'published'
      ? new Date().toISOString()
      : desiredStatus === 'scheduled' && scheduledAt
        ? scheduledAt
        : null;
    ...
    // Map preferred category from queue or AI suggestion
    const preferredCategorySlug = task.categorySlug || genData.suggested_category || null;
    if (preferredCategorySlug) {
      const { data: cats } = await supabase.from('categories').select('id, slug').eq('slug', preferredCategorySlug);
    ...
    if (idx !== -1) {
      const existingAttempts = updatedState.tasks[idx].attempts || 0;
      updatedState.tasks[idx].attempts = existingAttempts + 1;
      updatedState.tasks[idx].updatedAt = new Date().toISOString();

      if (canRetryTask(updatedState.tasks[idx])) {
        updatedState.tasks[idx].status = 'pending';
        updatedState.tasks[idx].error = `${err.message}`;
        updatedState.tasks[idx].currentStep = `Retrying (${updatedState.tasks[idx].attempts}/${updatedState.tasks[idx].maxAttempts ?? 3})`;
      } else {
        updatedState.tasks[idx].status = 'failed';
        updatedState.tasks[idx].error = err.message;
        updatedState.tasks[idx].currentStep = 'Failed after retries';
      }
...
app.post('/api/queue/add', (req, res) => {
  const { urls, instructions, categorySlug, postStatus, scheduledAt } = req.body;
  if (!urls || !Array.isArray(urls) || urls.length === 0) {
    return res.status(400).json({ error: 'urls array is required' });
  }

  const state = loadQueue();
  const seenUrls = new Set(state.tasks.map((t: QueueTask) => t.url.toLowerCase()));
  const newTasks: QueueTask[] = urls
    ...
    .filter(task => !seenUrls.has(task.url.toLowerCase()));

  newTasks.forEach(task => seenUrls.add(task.url.toLowerCase()));
...
// API: Delete selected tasks by IDs
app.post('/api/queue/delete', (req, res) => {
  ...
  const state = loadQueue();
  const idsToDelete = new Set(taskIds.map((id: string) => String(id).trim()));
  ...
  res.json({ success: true, deleted: deletedCount, remaining: state.tasks.length });
});

// API: Bulk delete posts by IDs
app.post('/api/admin/posts/bulk-delete', async (req, res) => {
  ...
  try {
    ...
    if (!postIds || !Array.isArray(postIds) || postIds.length === 0) {
      console.log('[BulkDelete] Invalid input - returning 400');
      return res.status(400).json({ error: 'postIds array is required' });
    }

    console.log(`[BulkDelete] Attempting to delete ${postIds.length} posts:`, postIds);

    // Check Supabase configuration
    if (!supabaseUrl || !supabaseKey) {
      console.error('[BulkDelete] Supabase not configured');
    ...
    for (const id of postIds) {
      try {
        console.log(`[BulkDelete] Deleting post ${id}...`);
        ...
        if (error) {
          console.error(`[BulkDelete] Failed to delete post ${id}:`, error);
          failed++;
        } else {
          console.log(`[BulkDelete] Successfully deleted post ${id}`);
          succeeded++;
        }
      } catch (deleteError: any) {
        console.error(`[BulkDelete] Error deleting post ${id}:`, deleteError);
        failed++;
      }
    }
    
    console.log(`[BulkDelete] Completed: ${succeeded} deleted, ${failed} failed`);
    ...
    console.log('[BulkDelete] Sending response:', response);
    ...
    if (!res.headersSent) {
      ...
      console.error('[BulkDelete] Sending error response:', errorMessage);
      return res.status(500).json({ 
    ...
    // If headers already sent, we can't do anything
    console.error('[BulkDelete] Response headers already sent, cannot send error response');
  }
});

app.get('/api/queue/scheduler', (req, res) => {
  res.json({
    enabled: autoSchedulerEnabled,
    intervalMs: autoSchedulerIntervalMs,
    intervalHours: autoSchedulerIntervalMs / (60 * 60 * 1000),
  });
});

app.post('/api/queue/scheduler', (req, res) => {
  const { enabled, intervalHours } = req.body || {};
  if (typeof enabled === 'boolean') {
    autoSchedulerEnabled = enabled;
  }
  if (typeof intervalHours === 'number' && intervalHours > 0) {
    autoSchedulerIntervalMs = intervalHours * 60 * 60 * 1000;
  }
  applyAutoSchedulerState();
  res.json({ enabled: autoSchedulerEnabled, intervalMs: autoSchedulerIntervalMs, intervalHours: autoSchedulerIntervalMs / (60 * 60 * 1000) });
});

app.post('/api/queue/publish-scheduled', async (req, res) => {
  try {
